...
- **O número muda a cada atualização da planilha, e muda muito.** Entre as duas versões que recebi, a fila caiu de 604 para 502 (−17%) porque 101 cancelamentos e 57 encerramentos foram registrados no intervalo. **Qualquer número desta seção é uma foto da data em que a planilha foi exportada**, não uma previsão estável. Fevereiro/27 sozinho perdeu 39% em uma atualização.
- **O que conta como fila é escolha explícita:** ficam fora "cancelado" e "encerrou plano"; ficam dentro "pendente de pagamento" e "bloqueado", porque ainda são contrato vivo. Se a régua comercial for outra, o número muda — a coluna de status por mês está na aba `Projecao_Terminos` para recalcular.
...
- **Os destinos e as datas das transferências são decisão sua, não registro da planilha.** Ana Elisa → Vinícius em out/26 e Carol Leão → Thiago já valendo vêm do que você me disse. A planilha continua mostrando o nome de quem saiu. A data importa tanto quanto o destino: com a virada em out/26 o pico do Vinícius é 74; se ela fosse hoje, seria 101, porque ele ainda não teria perdido as 22 de setembro.
- **Antes da virada, as 54 da Ana Elisa não pesam em nenhuma carteira individual.** Elas aparecem como `em_transito` na carga do time, então o total de 516 se preserva — mas o modelo assume que ela as conduz até out/26. Se ela sair antes, essa carga cai no time sem aviso.
- **6 das 13 da Carol Leão têm data de término já vencida com status ainda ativo.** Ou a planilha está velha, ou renovaram. Contei como ativas, porque o status é o campo que diz se ela é cliente hoje — mas o número real pode ser 7.
- **O campo `consultor` da matrícula erra por margem larga.** A Carol conferiu a carteira dela: 7, não 13. Só 5 batem nos dois cadastros. Confrontando cada consultor contra a própria aba, a concordância fica entre 65% e 87%, e o único caso verificado por uma pessoa é o pior de todos. Apliquei a correção só à Carol, porque as abas dos outros misturam Pró e Club enquanto o cadastro é só Club — a diferença pode ser erro ou pode ser recorte, e não tenho como distinguir. **Se a taxa da Carol valer para os demais, todas as carteiras individuais aqui estão infladas.**
- **20 das 41 alunas do Daniel não têm dono atribuído no modelo.** A aba dele registra o destino de 20; 2 iam para consultores que também saem e foram ao pool; as outras 19 não aparecem na aba. Você disse que já foram redistribuídas; a planilha não diz para quem. Elas entram na carga do time e ficam fora de toda linha individual — inventar o destino delas distorceria as carteiras de sete pessoas de uma vez. Consequência prática: **as carteiras individuais mostradas são um piso**, não o número real, para todos menos Vinícius e Thiago.
